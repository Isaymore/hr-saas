...
import { getPersonalDetail, updatePersonal, updateUserInfoById } from '@/api/employees'
import { getUserInfoById } from '@/api/employees'
export default {
  name: 'UserInfo',
  data() {
    return {
  ...
  methods: {
    async getPersonalDetail() {
      const result = await getPersonalDetail(this.userId) // 获取员工数据
    ...
    async saveUser() {
    //  调用父组件
      await updateUserInfoById(this.userInfo)
      this.$message.success('保存成功')
    },
